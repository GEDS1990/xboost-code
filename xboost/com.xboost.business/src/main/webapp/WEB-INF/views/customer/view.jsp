                ...
                <div class="col-lg-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            ...
                            <a href="javascript:;" id="editLink" class="btn btn-xs btn-info pull-right" style="margin-left:15px;">编辑</a>
                            ...
                            <c:choose>
                                <c:when test="${not empty customer.userid}">
                                    <a href="javascript:;" id="publicCustomer" class="btn btn-xs btn-warning pull-right"
                                       style="margin-left:15px;">公开客户</a>
                                    <a href="javascript:;" id="tranCustomer" class="btn btn-xs btn-primary pull-right"
                                       style="margin-left:15px;">转交客户</a>
                                ...
                                <c:otherwise>
                                    <a href="javascript:;" id="getCustomer" class="btn btn-xs btn-warning pull-right"
                                       style="margin-left:15px;">加入名下</a>    
                                </c:otherwise>
                            </c:choose>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="/customer/progress/new" method="post" id="newForm" enctype="multipart/form-data">
                    ...
                    <div class="form-group" id="fileControls" style="display:none;">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="" id="editCustomerForm">
                    ...
                    <div class="form-group">
                        <label>客户姓名</label>
                        <input type="text" class="form-control" value="${customer.custname}" name="custname" id="custname">
                    </div>
                    <div class="form-group">
                        <label>联系人</label>
                        <input type="text" class="form-control" value="${customer.contact}" name="contact" id="contact">
                    </div>
                    <div class="form-group">
                        <label>联系电话</label>
                        <input type="text" class="form-control" value="${customer.tel}" name="tel" id="tel">
                    </div>
                    <div class="form-group">
                        <label>联系地址</label>
                    ...
                    <div class="form-group">
                        <label>微信</label>
                        <input type="text" class="form-control" value="${customer.wechar}" name="wechar" id="wechar">
                    </div>
                    <div class="form-group">
                        <label>QQ</label>
                        <input type="text" class="form-control" value="${customer.qq}" name="qq" id="qq">
                    </div>
                    <div class="form-group">
                        <label>备注</label>
                        <input type="text" readonly="true" class="form-control" value="${customer.mark}" name="mark" id="mark">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button type="button" id="updateCustomerBtn" class="btn btn-primary">保存</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<script>
    $(function () {
        ...
        $("#editLink").click(function(){
            ...
            var id = ${customer.id};
            $.post("/customer/customer.json",{"id":id}).done(function(result){
                $("#id").val(result.customer.id);
                $("#custname").val(result.customer.custname);
                $("#contact").val(result.customer.contact);
                $("#tel").val(result.customer.tel);
                $("#address").val(result.customer.address);
                $("#email").val(result.customer.email);
                $("#wechar").val(result.customer.wechar);
                $("#qq").val(result.customer.qq);
                $("#mark").val(result.customer.mark);
            }).fail(function(){
        ...
        $("#updateCustomerBtn").click(function(){
            $.post("/customer/update",$("#editCustomerForm").serialize()).done(function(result){
                if(result == "success") {
                    $("#editCustomerForm")[0].reset();
        ...
        <c:choose>
            ...
            <c:otherwise>
                ...
                $("#getCustomer").click(function () {
                    if (confirm("将该客户加入自己名下后，其他用户都看不到该客户的信息,确定吗")) {
                        $.post("/customer/get/${customer.id}").done(function (result) {
                            if (result.state != "success") {
                                alert(result.message);
                            } else {
                                window.history.go(0);
