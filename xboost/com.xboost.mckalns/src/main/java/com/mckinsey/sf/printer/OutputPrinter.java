...
//					return ok;
	...
	public static void PrintProblem(Solution s) {
		...
		for(Route r : s.getRoutes().values()){
			...
			totalLoad += r.getJobs().size()*1.0 / r.getC().getDimensions()[0];
...
//				int indicator = Route.getIndicator(cur,  r.getActs().get(0), r.getActs().get(r.getActs().size()-1));
			...
			if(Main.carsMap.containsKey(r.getC().getType())){
				Main.carsMap.put(r.getC().getType(), Main.carsMap.get(r.getC().getType())+1);
			}else{
				Main.carsMap.put(r.getC().getType(), 1);
			}
		...
		for(Entry<String, Integer> keyValue : Main.carsMap.entrySet()){
			String key = keyValue.getKey();
			int value = keyValue.getValue();
			
			printLine( "|"+key+"\t|"+ (Main.carsMap.containsKey(key)?value:0)+"\t\t\t|");
...
//						}else{
						rr.createCell(8).setCellValue(Math.max(arrTime, Main.totalJobs.get(jobId).getPickup().getTw().getStart()));
							...
							for(;j<newActs.size();j++){
								...
								if(curJ.getLocation().equalsIgnoreCase(cur.getLocation())){
									if(type.equals(curJ.getType())){
										count1 ++;
										sbLoc.append(Main.totalJobs.get(curJ.getJobId()).getDelivery().getLocation()+"/");
										sbVol.append(Main.totalJobs.get(curJ.getJobId()).getDimensions()[0]+"/");
										currentLoc.add(Main.totalJobs.get(curJ.getJobId()).getDelivery().getLocation());
										
										//TODO: update sheet2
										Row rrr4 = sheet2.createRow(count2++);
										rrr4.createCell(0).setCellValue(Main.totalJobs.get(curJ.getJobId()).getPickup().getLocation());
										rrr4.createCell(1).setCellValue(Main.totalJobs.get(curJ.getJobId()).getDelivery().getLocation());
										...
										rrr4.createCell(5).setCellValue(Main.totalJobs.get(curJ.getJobId()).getDimensions()[0]);
									}
									//update sheet2 end
								}else{
									break;
