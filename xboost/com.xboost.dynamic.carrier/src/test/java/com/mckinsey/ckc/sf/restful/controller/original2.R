...
##########################################################################
###Eric Fan @ 2016/12/29 1:42pm##########################################
###code version two#######################################################
###stage 2 model: added initial carrier allocation based on initial demand
##########################################################################
###Frank Lai @ 2016/12/29 10:20pm##########################################
###code version two#######################################################
###stage 2 model: delete data with O euqals D
##########################################################################
###Frank Lai @ 2017/03/23 10:20pm##########################################
###code version two#######################################################
###stage 3 model: remove 
##########################################################################
...
require(data.table)
model_version <- "v8.2"
source(paste("./dynamic network_function_definition_",model_version,".R",sep=""))
...
for(N_carrier in N_carrier_range) {
  ...
  carriers_tbl_all$speed <- carrier_speed_mm
  carriers_tbl_all$sleep_turns <- 0
  carriers_tbl_all$carrier_dest_lon<- as.numeric(0)
  carriers_tbl_all$carrier_dest_lat<- as.numeric(0)
  carriers_tbl_all$group_id<-0
  carriers_tbl_all$carrier_parcel_mean_lon <- carriers_tbl_all$carrier_current_lon
  carriers_tbl_all$carrier_parcel_mean_lat <- carriers_tbl_all$carrier_current_lat
  carriers_tbl_all$carrier_parcel_meantime_to_dest <- as.numeric(0)
  carriers_tbl_all$carrier_theta <- 361
  ...
  parcels_tbl_all$parcel_theta<-180/pi*asin((parcels_tbl_all$parcel_dest_lat-parcels_tbl_all$parcel_origin_lat)/sqrt((parcels_tbl_all$parcel_dest_lon-parcels_tbl_all$parcel_origin_lon)^2+
  ...
  for(t in (picking_start_time*60/time_unit):(picking_end_time*60/time_unit)) {  
    ...
    if (t == picking_start_time_swot*60/time_unit){
      ...
      colnames(carriers_tbl_all_swot)<-c("carrier_initial_lon","carrier_initial_lat",
                                         "carrier_current_lon","carrier_current_lat","carrier_id")
      carriers_tbl_all_swot$speed <- carrier_speed_mm
      carriers_tbl_all_swot$sleep_turns <- 0
      ...
      carriers_tbl_all_swot$carrier_parcel_mean_lon <- carriers_tbl_all_swot$carrier_current_lon
      carriers_tbl_all_swot$carrier_parcel_mean_lat <- carriers_tbl_all_swot$carrier_current_lat
      carriers_tbl_all_swot$carrier_parcel_meantime_to_dest <- as.numeric(0)
      carriers_tbl_all_swot$carrier_theta <- 361
    ...
    delivers<-filter(parcels_tbl_all,
                     (parcel_deliver_status==1)&(parcel_time_id_initial<=t)&
                       (abs(parcel_dest_lon-parcel_current_lon)==0)&
                       (abs(parcel_dest_lat-parcel_current_lat)==0))       
    parcels_tbl_all[parcels_tbl_all$parcel_id%in%delivers$parcel_id,"parcel_deliver_status"] <-2
    parcels_tbl_all[parcels_tbl_all$parcel_id%in%delivers$parcel_id,"parcel_deliver_time"] <-t
    parcels_tbl_all[parcels_tbl_all$parcel_id%in%delivers$parcel_id,"mintime_to_dest"]<-0
    
    
    
    #calculate mean direction
    carriers_tbl_all[,c("carrier_parcel_mean_lon","carrier_parcel_mean_lat","carrier_parcel_meantime_to_dest")] <- sqldf(
      "select avg(y.parcel_dest_lon) as carrier_parcel_mean_lon, avg(y.parcel_dest_lat) as carrier_parcel_mean_lat, ifnull(avg(y.mintime_to_dest),0) as carrier_parcel_meantime_to_dest
      from carriers_tbl_all x left join parcels_tbl_all y
      on x.carrier_id = y.parcel_carrier_id and y.parcel_deliver_status = 1
      group by x.carrier_id
      ")
    
    carriers_tbl_all$carrier_theta <- 361
    unna_site <- which(!is.na(carriers_tbl_all$carrier_parcel_mean_lon))
    carriers_tbl_all$carrier_theta[unna_site] <- 
      (180/pi*asin((carriers_tbl_all$carrier_parcel_mean_lat-carriers_tbl_all$carrier_current_lat)/
                     sqrt((carriers_tbl_all$carrier_parcel_mean_lon-carriers_tbl_all$carrier_current_lon)^2+(carriers_tbl_all$carrier_parcel_mean_lat-carriers_tbl_all$carrier_current_lat)^2)))[unna_site]
    ...
    opportunities$current_distance <- sqrt((opportunities$carrier_current_lon-opportunities$parcel_current_lon)^2+
                                             (opportunities$carrier_current_lat-opportunities$parcel_current_lat)^2)*distance_inflator
    #calculate current distance between carrier and parcel
    opportunities<-opportunities %>%
      group_by(carrier_id) %>%
      top_n(n=-interest_rank,wt=current_distance)
    #calculate angle between potential parcel direction and mean direction for parcels on carrier
    opportunities$angle_between <- (opportunities$carrier_theta<=360)*abs(opportunities$parcel_theta-opportunities$carrier_theta)
    opportunities$angle_between <- pmin(360-opportunities$angle_between,opportunities$angle_between)
    opportunities<-filter(opportunities,angle_between<=opt_max_angle)
    ...
    opportunities$increment_time <- third_side(opportunities$mintime_to_dest,
                                               opportunities$carrier_parcel_meantime_to_dest,opportunities$angle_between)+
      opportunities$mintime_to_dest-opportunities$carrier_parcel_meantime_to_dest
    ...
    for(n in 1:N) {
      ...
      delivers_n <- delivers[delivers$parcel_carrier_id==id_n,]
      carrier_n <- carriers_tbl_all[carriers_tbl_all$carrier_id==id_n,]
      #mark: to be delete
      carriers[[n]]@parcel_list$mintime_to_dest <- sqrt((carriers[[n]]@position_current[1]-carriers[[n]]@parcel_list$parcel_dest_lon)^2+
                                                    (carriers[[n]]@position_current[2]-carriers[[n]]@parcel_list$parcel_dest_lat)^2)*
        distance_inflator/carriers[[n]]@speed
      # print(delivers_n)
      if(nrow(delivers_n)>=1) {
        for(delivers_i in 1:nrow(delivers_n)) {
          # carriers[[n]]<-dropoff(carriers[[n]],delivers_n$parcel_id[delivers_i])
          carriers[[n]]@parcel_list<-filter(carriers[[n]]@parcel_list,parcel_id!=delivers_n$parcel_id[delivers_i])
        }}
      
      ###calculate carriers' theta
      carriers_n_theta <- carriers_tbl_all$carrier_theta[n]
      
      #opportunity of this carrier
      opportunities_n<-filter(opportunities,((carrier_id==id_n)&(parcel_deliver_status==0))|
                                ((carrier_id==id_n)&(parcel_deliver_status==-1)&(parcel_carrier_id==id_n)))
      ...
      if (t == (picking_start_time*60/time_unit)){
        ...
        pickups1<-filter(opportunities_n,(current_distance==0)) } else {
          ...
          pickups1<-filter(opportunities_n,((current_distance==0)&(angle_between<=max_angle0||
                                                                     (increment_time<=(10^((t - picking_start_time*60/time_unit)/((picking_end_time*60/time_unit)-(picking_start_time*60/time_unit))-1))*
                                                                        carrier_parcel_meantime_to_dest))))
      ...
      pickups2<-filter(parcels_tbl_all,(parcel_deliver_status==-1)&
                         (parcel_origin_lon==carrier_n$carrier_dest_lon)&
                         (parcel_origin_lat==carrier_n$carrier_dest_lat)&
                         (parcel_carrier_id==id_n))
      ...
      if(length(pickup_id)>=1) {
        for(pickup_i in 1:length(pickup_id)) {
          if(current_item_cnt>carrier_capacity*(carrier_n$group_id==0)+swot_capacity*(carrier_n$group_id==1)) break
          # carriers[[n]]<-pickup(carriers[[n]],pickup_id[pickup_i])
          #update parcel
          parcels_tbl_all[parcels_tbl_all$parcel_id==pickup_id[pickup_i],"parcel_carrier_id"] <- id_n
          parcels_tbl_all[parcels_tbl_all$parcel_id==pickup_id[pickup_i],"parcel_deliver_status"] <- 1
          parcels_tbl_all[parcels_tbl_all$parcel_id==pickup_id[pickup_i],"parcel_pickup_time"] <- t
          if(nrow(opportunities)>0) {
            opportunities[opportunities$parcel_id==pickup_id[pickup_i],"parcel_carrier_id"] <- id_n
            opportunities[opportunities$parcel_id==pickup_id[pickup_i],"parcel_deliver_status"] <- 1
          }
          #update carrier
          new_parcel=data.frame(parcel_id=pickup_id[pickup_i],
                                parcel_volume=parcels_tbl_all[parcels_tbl_all$parcel_id==pickup_id[pickup_i],"parcel_volume"],
                                parcel_dest_lon=parcels_tbl_all[parcels_tbl_all$parcel_id==pickup_id[pickup_i],"parcel_dest_lon"],
                                parcel_dest_lat=parcels_tbl_all[parcels_tbl_all$parcel_id==pickup_id[pickup_i],"parcel_dest_lat"],
                                mintime_to_dest=parcels_tbl_all[parcels_tbl_all$parcel_id==pickup_id[pickup_i],"mintime_to_dest"])
          carriers[[n]]@parcel_list<-rbind(carriers[[n]]@parcel_list,new_parcel)
          #reorder current parcels on the carrier
          parcel_list_temp<-carriers[[n]]@parcel_list
          parcel_list_temp<-parcel_list_temp[with(parcel_list_temp, order(mintime_to_dest)), ]
          carriers[[n]]@position_dest<-c(parcel_list_temp$parcel_dest_lon[1],parcel_list_temp$parcel_dest_lat[1])
          carriers[[n]]@parcel_list<-parcel_list_temp[,c("parcel_id","parcel_volume","parcel_dest_lon",
                                                   "parcel_dest_lat","mintime_to_dest")]
      ...
      #mark: enable this later
      #operating time
      # if(carriers[[n]]@sleep_turns==1) {
      #   carriers[[n]]@sleep_turns<-0
      #   next}
      # if(length(pickup_id)+nrow(delivers_n)>=1) {
      #   carriers[[n]]@sleep_turns<-0}
      ...
      if(current_item_cnt==0) {
        carrier_urgency<--999999
      } else if(current_item_cnt>=carrier_capacity*(carrier_n$group_id==0)+swot_capacity*(carrier_n$group_id==1)) {
        carrier_urgency<-Inf}
      else {
        #mark:full loaded scenario
        min_remaining_time<-min(parcels_tbl_all[(parcels_tbl_all$parcel_deliver_status==1)&
                                                  (parcels_tbl_all$parcel_carrier_id==id_n),"parcel_deadline"])-t
      ...
      carriers_tbl_all[carriers_tbl_all$carrier_id==id_n,
      ...
      # parcels_tbl_all[parcels_tbl_all$parcel_deliver_status==1,c("parcel_current_lon","parcel_current_lat")] <- sqldf(
      #   "select y.carrier_current_lon,y.carrier_current_lat
      #   from parcels_tbl_all x left join carriers_tbl_all y
      #   on x.parcel_carrier_id = y.carrier_id
      #   where x.parcel_deliver_status = 1
      #   "
      # )
      # record carrier track
      this_carrier_records <- data.frame(time_id=t,
                                         carrier_id=carriers[[n]]@ID,
                                         carrier_group_id=carriers[[n]]@group_id,
    ...
    # parcels_tbl_all$mintime_to_dest[parcels_tbl_all$parcel_deliver_status==1] <- sqrt((parcels_tbl_all$parcel_current_lon-parcels_tbl_all$parcel_dest_lon)^2+
    #                                                                                          (parcels_tbl_all$parcel_current_lat-parcels_tbl_all$parcel_dest_lat)^2)[parcels_tbl_all$parcel_deliver_status==1] *
    #   distance_inflator/carrier_speed_mm # mark: need to update the speed
    parcels_tbl_all$mintime_to_dest <- sqrt((parcels_tbl_all$parcel_current_lon-parcels_tbl_all$parcel_dest_lon)^2+
                                                                                        (parcels_tbl_all$parcel_current_lat-parcels_tbl_all$parcel_dest_lat)^2) *
      distance_inflator/carrier_speed_mm # mark: need to update the speed
    ...
    print(paste("carrier number=",N,", time=",t,", picked up ",
                ...
                ", ontime delivered ",
                as.integer(sum(parcels_tbl_all$parcel_volume[parcels_tbl_all$parcel_deliver_status==2&parcels_tbl_all$parcel_deliver_time <= parcels_tbl_all$parcel_deadline])),
                ", unattended ",
...
write.csv(carrier_records,paste("dynamic_network_output/",model_version,"/carrier_capacity_",carrier_capacity,
                                "_N_carrier_",N_carrier,"_",N_swot_carrier,"_carrier_records.csv",sep=""))
write.csv(parcels_tbl_all,paste("dynamic_network_output/",model_version,"/carrier_capacity_",carrier_capacity,
                                "_N_carrier_",N_carrier,"_",N_swot_carrier,"_parcel_records.csv",sep=""))
