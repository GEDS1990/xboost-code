...
public interface IConstantsArgs {
	
	boolean RECORD_RESULT = false; //whether to record carrier records and parcel info into database
	
	int CARRIER_SUPPLY = 9999; // unlimited carrier resource
	int CARRIER_CAPACITY = 20;//12; // unlimited carrier capacity
	int TIME_UNIT = 5;//1;
	int PICKUP_TIME = 0;
	int DROPOFF_TIME = 0;
	int PICKING_START_TIME = 10;     //------pickup start time
	int PICKING_END_TIME = 24;        //------pickup end time
	int DEMAND_CUT_TIME = 18;         //------Demand cut time
	int SIMU_HOUR = 8;
	...
	int N_CARRIER_RANGE = 500;//800;    //--------Normal Carrier :
	int[] SPEED_RANGE = {20};//{10};
	int OPT_MAX_ANGLE = 45;//30;
