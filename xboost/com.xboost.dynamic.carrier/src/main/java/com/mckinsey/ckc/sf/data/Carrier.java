...
public class Carrier implements IConstants {
	...
	public void pickUp(int pickupID, int timeid,List<Opportunity> oppotunities) {
...
//			for (Opportunity op : oppotunities) {
...
//		}
	...
	public void dropOff(Parcel parcel, int t,List<Opportunity> oppotunities) {
		// update parcel
...
//		}
	...
	public void think(List<Opportunity> oppotunitiesN, int timeId,List<Opportunity> oppotunities) {
...
//			System.out.println("carrier "+carrierID+" next choice is top parcel on parcel list:"+topParcel.getParcelID());
		} else if (scenario == 2) {
...
//			System.out.println("carrier "+carrierID+" next choice is max urgency parcel on opprtunities:"+maxUrgencyIndex);
		} else {
			// stand();
//			System.out.println("carrier "+carrierID+" stand still.");
		}
	}

	public void thinkDropOff(List<GroupInfo> groupList, List<Opportunity> oppotunitiesN, int timeId,List<Opportunity> oppotunities) {
		int Nopt = oppotunitiesN.size();
		double maxUrgency = Double.MIN_VALUE;
		int maxUrgencyIndex = 0;
		for (Opportunity op : oppotunitiesN) {
	...
	public void thinkPickUp(List<GroupInfo> groupList, List<Opportunity> oppotunitiesN, int timeId,List<Opportunity> oppotunities) {
		int Nopt = oppotunitiesN.size();
		double maxUrgency = Double.MIN_VALUE;
		int maxUrgencyIndex = 0;
		for (Opportunity op : oppotunitiesN) {
	...
	//Synchronize think
	//TODO
	public static void synchronize_think(int timeId,List<Opportunity> oppotunities,HashMap<Integer, Carrier> carrierMap){
		
		List<UrgencyScoreCalculator> urgencyList = buildUrgency(oppotunities, timeId);
		...
		for(Map.Entry<Integer, Carrier> carrier : carrierMap.entrySet()){
			...
			if((c.oppoNSize+c.parcelList.size()) == 0){
				//stand still
//				System.out.println("carrier "+c.getCarrierID()+" stand still");
...
//				System.out.println("initialized carrier "+c.getCarrierID()+"  next choice is top parcel on parcellist:"+topParcel.getParcelID());
			}
		}
		...
		for(UrgencyScoreCalculator calculator : urgencyList){
			...
			carrierMap.get(carrier.getCarrierID()).setDestPos(parcel.getOrigPos());
			// if this carrier has reserved some other parcel, cancel that
			// reservation
			for (Entry<Integer, Parcel> pEntry : Main.parcelMap.entrySet()) {
				Parcel p = pEntry.getValue();
...
//			System.out.println("carrier "+carrier.getCarrierID()+" next choice is max urgency parcel on opprtunities:"+parcel.getParcelID());
...
//				cUrgencyTime = (REQUIRED_SHIPPING_TIME - 2 - minRemainingTime)*TIME_UNIT;
				cUrgencyTime = -Math.abs(minRemainingTime)*TIME_UNIT;
			}
			double pMinParcelDest = 0;
			if(minParcel != null)
				pMinParcelDest = Math.sqrt(Math.pow(minParcel.getDestPos().getX() - parcel.getDestPos().getX(), 2)
					+ Math.pow(minParcel.getDestPos().getY() - parcel.getDestPos().getY(), 2)) * DISTANCE_INFLATOR/speed;
...
//			}
			pUrgencyTime =-Math.abs(parcel.getDeadline()-timeId)*TIME_UNIT;
			this.urgencyScore = a*distTime + b*incremenTime + c*cUrgencyTime + d*pUrgencyTime ;
//			this.urgencyScore = (-distTime-parcel.getMinTimeToDest()+pUrgencyTime)+
//					 (-distTime-parcel.getMinTimeToDest()-pMinParcelDest+cUrgencyTime);
		}
